...
export default {
  ...
  computed: {
    // Tính tổng số trang
    totalPages() {
      if (!this.sach || this.sach.length === 0) return 0;
      return Math.ceil(this.sach.length / this.pageSize);
    },
    
    // "Cắt" mảng sach để lấy dữ liệu cho trang hiện tại
    paginatedData() {
      if (!this.sach || this.sach.length === 0) return [];
      const start = (this.currentPage - 1) * this.pageSize;
      const end = start + this.pageSize;
      return this.sach.slice(start, end);
    },
   ...
   pageNumbers() {
    ...
    pageInfo() {
      ...
      const total = this.sach.length;
      
      // Nếu không có gì, trả về thông báo rỗng
     if (!this.sach || this.sach.length === 0) {
  ...
  emits: ["update:activeIndex","delete:sach"],
  methods: {
    changePage(page) {
    ...
    deleteSach(id) {
      ...
      if (confirm("Bạn muốn xóa sách này?")) {
        this.$emit("delete:sach", id); // Chỉ phát tín hiệu với ID
      }},
  },
  watch: {
...
<template>
    ...
    <div class="app-title">
      <ul class="app-breadcrumb breadcrumb side">
        <li class="breadcrumb-item active"><a href="#"><b>Danh sách của sách</b></a></li>
      </ul>
      <div id="clock"></div>
    </div>
...
<tbody>
    <tr 
        v-for="(sach, index) in paginatedData" 
        :key="sach._id" 
    ...
    <img 
        v-if="sach.hinhAnh" 
        :src="'http://localhost:3000/uploads/' + sach.hinhAnh" 
        alt="Ảnh sách"
        width="80" 
        height="100"
    ...
    <button 
      ...
      @click.stop="deleteSach(sach._id)"
    >
